...
| G-39 | Empty states G-14 missed | Remote "No devices paired yet." (`text-ink-dim`, brighter than the rest), AI Workspace files, Agent Status skills, Performance incidents: hand-rolled | `EmptyState` | done (inline size in panels, list size for the empty Incidents tab; Remote's "Paired Devices" heading Title Case). Kept: Welcome's "No tabs open." (the app's first-run card with its own layout) and the goal-loop overlay's heading |
