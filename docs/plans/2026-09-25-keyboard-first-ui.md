...
| G-40 | "directory" left in UI copy after G-24 | the New Tab picker title ("Working Directory"), New Agent In's disabled reason, Explorer's read-error fallback | "folder" (G-24) | done. Kept: `controlReference` / `controlInteractions` text (agent-facing MCP documentation, not UI), bufferOps' error matchers (they match OS error text) |
