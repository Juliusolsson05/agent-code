...
| G-37 | Grid Dispatch Simple / Advanced | the last hand-drawn two-state pair after G-10: text buttons around "│", with accent text as the only "on" look | `SegmentedControl` | done (`pressed` semantics, because switching resizes the dialog; aria-pressed kept, and the existing test holds) |
