...
| K2-4 | Feed scroller (known N13, `feed/ui/Feed.tsx:1179`) | not focusable; keyboard scroll never counts as engagement; Reply-to-Selection needs a mouse selection | handled in N13 | scroller + engagement done in N13; keyboard quoting done as **Reply to Reader Message** (UNCONFIRMED for the owner; ask-3 default, steering k11). It is a palette-only command with no new default chord: it quotes Reader Mode's selected message (⌥↑ / ⌥↓ pick it) into the composer of the agent being read, through the same `prefixDraftWithQuote`. The draft is kept beneath the quote, nothing is sent, and other agents' drafts are untouched. It is unavailable with no message shown or with Reader closed. Reader publishes the selection through `readerMessageStash` (module scope, like selectionStash). The tests fail 3/3 on the old wiring, and four mutations are caught (draft dropped, reader guard removed, grid-focus target, unmount not clearing). Steering k12 found a wrong-agent case: after a Reader switch from A to B, the stash still named A until B published, so the command quoted A's old message into A's draft. The published message must now belong to `readerMode.focusedSessionId`. A fail-first test covers the lag and B's normal publication afterwards |
...
| G-31 | Casing in labels defined as data | G-23 scanned JSX text, so `label:` props slipped: the browser pocket's native menus (all sentence case, while the app menu and session menu are Title Case), Skills row menu, CLI banner actions, Notify Only / All Limits choices, and the segmented options All Projects / Selected Projects, Show All (their siblings, e.g. This Folder / Repository, were already Title Case) | the casing ruling | done (18 pocket menu labels, 3 Skills items, 2 banner actions, 2 choices, 3 segmented options, Workflows' Show All, the pocket's Fit Pane). Kept: persisted data labels ("System default" is stored as the device label), feed tool labels (G-29 voice), key/value field labels in Agent Status. Ruling (round-2 review C1): duration and range option lists (Analytics "24 hours | 7 days | 30 days | All time", Performance "15 min … 7 days", report ranges) stay sentence case because they read as values, not commands. The "All Time" this row first changed broke that list's own consistency and was reverted |
...
| G-33 | Error fallback wording | "Could not …" ×58 vs "Failed to …" ×18 / "Unable to …" ×2, with and without a final period | "Could not …." | done (18 renderer fallbacks, the spawn fallback, and Path Picker's listing error). Kept: "Failed to start" (a row status, not a message) and main-process messages Round-2 review C2 counted 29 more "Could not …" strings without the final period. The 18 that end in plain words got it; the ones ending in an interpolated error or detail (`: ${error}`) keep that text's own punctuation, and main/runtime messages are outside this renderer row |
...
| G-35 | Degraded-run banners (RestoreBanner, CLI update banner) | Hide ↔ corner chip unmounted the pressed control, so focus fell to <body> (the 60 s auto-collapse too); RestoreBanner used raw `bg-warning/15` / `/10` and a borderless Hide beside the CLI banner's bordered current-colour actions | focus carried to the counterpart; `-soft` tokens; one action grammar | done (now through the shared `useSwapFocus`, G-36; focus moves chip ↔ Hide only when it was inside the banner, so auto-collapse never takes a composer's caret; `bg-warning-soft`; Hide uses the CLI banner's control). CLI banner dismiss left as is: its row unmounts to <body>, which type-to-focus already reclaims for the focused pane, and a single row is the normal case |
| G-36 | Feed disclosures that swap controls | pagers rendered previous/next only while they had somewhere to go, so paging onto an end unmounted the focused button (×3: CodeBlock, PagedTextViewer, multi-edit); "(show all)" ↔ "collapse" and "view paged content" ↔ pager unmounted the pressed control. Focus fell to <body>, and Tab restarted at the top of the document | keep focus through a swap | done (`FeedPagerButtons`: both always mounted, an end is `aria-disabled` and a no-op, since `disabled` drops focus too; `useSwapFocus` carries focus to the counterpart only when the pressed control had it; expanding output does not scroll to the far "collapse"; opening paged code focuses "next"). Fail-first: 4 of the 5 new tests fail on the pre-change files Round-2 reviews A, B and C: the no-op was unasserted. The test now checks that the page and range are unchanged after a click on a disabled end, in both directions. The `hasNext` guard mutation is caught. The `hasPrevious` guard is an equivalent mutant, because every caller already clamps at the start (`length > 1 ? … : current`, `Math.max(0, …)`) |
| G-37 | Single-choice pairs missed by G-10 | Grid Dispatch Simple / Advanced (text buttons around "│", with accent text as the only "on" look); Performance › History range (secondary/ghost Buttons); "Back to live" not Title Case | `SegmentedControl` | done (both `pressed`: Grid's switch resizes the dialog, and each range change reads history from disk; the range group is pinned to h-7 to sit level with its toolbar's sm Buttons; "Back to Live"). Kept: Path Picker's provider pills (each carries a missing-provider title and data flag the primitive does not model), Conversations' provider filter chips (multi-select toggles, not one choice) |
| G-38 | Sentence-case button labels G-23 missed | a scan of every button's text for multi-word labels with a lowercase content word found 36 in settings rows, Skills, the browser pocket, Performance, the key vault, extensions and the pane header ("Reset state", "Keep my draft", "Take over", "Refresh view", "Show top 10"…) | the casing ruling | done (36 labels plus MCP's "Copy In", a phrasal-verb particle that Title Case capitalizes, unlike a real preposition such as "Show in Spotlight";, their template partners Show All N / Show N Hidden, and every test query that named them. Two were `queryBy…toBeNull()` checks that would have passed vacuously on a missed rename). Kept on purpose: full-sentence confirmations ("I verified the transcript — allow sending again"), capture-field prompts ("Press any key or shortcut"), transient status in a button ("Copy failed"), feed tool labels (G-29), the provider's own option wording (Claude's "Yes, I trust this folder"). Ruling: confirm-dialog titles that are QUESTIONS stay sentence case ("Discard theme changes?", "Turn X off?"). All 16 already agree, a question reads as a sentence, and the HIG treats full-sentence alert titles that way. Cost if wrong: 16 strings |
| G-39 | Empty states G-14 missed | Reader's "no assistant message yet" (lowercase, no period), Remote "No devices paired yet." (`text-ink-dim`, brighter than the rest), AI Workspace files, Agent Status skills, Performance incidents: hand-rolled | `EmptyState` | done (inline size in panels, list size for the empty Incidents tab and Reader; Remote's "Paired Devices" heading Title Case). Kept: Welcome's "No tabs open." (the app's first-run card with its own layout) and the goal-loop overlay's heading Round-2 review C4 found two more, now on EmptyState: Dictation's "No microphones found." and Agent MCP Servers' "No MCP servers apply to this agent." |
| G-40 | "directory" left in UI copy after G-24 | the New Tab picker title ("Working Directory"), New Agent In's disabled reason, Explorer's read-error fallback, the pane header's "no project dir" tooltip | "folder" (G-24) | done. Kept: `controlReference` / `controlInteractions` text (agent-facing MCP documentation, not UI), bufferOps' error matchers (they match OS error text) |
| G-41 | Goal-loop strip and overlay controls | Pause ↔ Resume and Stop → Dismiss are separate conditional Buttons, so the pressed one unmounted once main reported the new phase, and focus fell to <body>, out of the overlay that owns every key while latched | keep focus through the swap | done (`useSwapFocus` keyed on the phase, one carrier per surface so focus stays in the strip or the overlay that was pressed; the hook now carries only if focus is still unowned when an ASYNC swap lands, and accepts a container whose first control takes focus). Dismiss removes the strip entirely, so nothing is carried; type-to-focus reclaims the pane |
...
- **G-36 Feed pagers:** long tool output now always shows both "previous"
  and "next"; the unavailable one is dimmed (it used to vanish). **Check**
  the dimmed end reads as unavailable, not broken.
- **K2-4 Reply to Reader Message (UNCONFIRMED, new command):** in Reader
  Mode, pick a message with ⌥↑ / ⌥↓, open the palette, and run "Reply to Reader
  Message". The agent's composer gets the quote above your draft; nothing is
  sent. **Decide** whether to keep it, and whether it deserves a chord.
- **G-29…G-33 Copy and small controls:** the browser pocket's right-click
  ...
  range options stay values in sentence case ("24 hours", "All time"); search boxes all end in "…"; error fallbacks (the ones ending in plain words) read "Could not
