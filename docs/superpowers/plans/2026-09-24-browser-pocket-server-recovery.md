...
3. Wake only that agent with a dedicated `browser-pocket.restart-request` WakeCaller, added to `src/shared/lifecycle/events.ts`. Do not change input-ready waits, interrupt the agent, or clear its native draft. Pass the stricter `requireEmptyNativeComposer` option through the existing delivery IPC: Codex's legacy readiness accepts occupied input, so generated tasks need a provider-owned check immediately before the reserved write.
...
- Chromium error-document navigation (`httpResponseCode === -1`) preserves the connection failure and receipt. Response evidence comes from `did-frame-navigate`; `<webview>`'s `did-navigate` carries only the URL. A real main-frame response ends that episode. Same-URL failed reloads keep the duplicate-send guard.
- Independent review found and corrected the webview event mismatch above, explicit no-write refusals being mislabeled uncertain, and Codex's legacy readiness accepting an occupied native draft. The generated-task option now refuses occupied or unverifiable Codex input without writing. A plain snapshot cannot distinguish placeholder prose from identical human text, so those cases require sending through the agent manually. Richer native draft observation remains tracked in #800; this change does not guess or clear native input.
- The initial 15 focused browser, shared URL and agent-control suites passed: 179 tests, followed by full CI. Review regressions additionally exercise native Codex readiness/delivery and preload-to-main option forwarding; final verification is recorded on the PR.
