...
- New tests for `runPrunePasses` with in-memory artifacts and a recording
  ...
  trims the oldest inactive artifacts of an over-cap bucket and skips the
  manual bucket, protected and active artifacts; the budget pass trims
  oldest-first until under budget; an artifact removed by one pass is never
  offered to a later pass; result totals match.
