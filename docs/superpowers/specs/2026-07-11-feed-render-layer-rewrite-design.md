...
- **Sealed-line cache:** input text is split at `\n`. Every fully-received line is highlighted once via `hljs.highlight(line, { language, continuation })` where `continuation` is the hljs continuation state carried from the previous line (handles template literals, block comments, JSX). Cache entry = `{ html, endState }` keyed by line index; a sealed line's cache entry is invalidated only if the line's text or its *incoming* state changed (normal streaming: never — text is append-only). Rendered as one `<span class="line">` per line with `dangerouslySetInnerHTML`, inside a single `<pre>`.
- **The live tail line** re-renders per delta (tiny — one line of tokenization), continuing from the last sealed state.
