...
The state file records a hash for every successful materialization and whether
Agent Code created the leaf skill directory. Before changing a provider file,
the service durably records a pending operation containing its path, previous
hash, desired hash, and any fingerprint-bound overwrite approval. This
write-ahead record is the only evidence that permits generated bytes to be
...
files. It unlinks only an exact hash-proven regular file. It removes a leaf
directory with non-recursive `rmdir` only when empty and recorded as created by
Agent Code. Externally modified files remain conflicts.
