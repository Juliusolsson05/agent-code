...
3. Compare that history with the target model's usable context budget.
...
The parser owns semantic history and capacity assessment. It does not own live
processes or UI. Assessment has three meaningful outcomes:
...
records to the source transcript. This is deliberate and user-visible. The UI
announces that compaction is required, and the switch remains locked until a
durable compaction record appears or the operation fails.
...
- **Codex → Claude:** first let Codex run native `/compact`. Then ask the now
  compacted live Codex session, read-only and without tools, to write a detailed
  portable handoff. Persist that assistant response as the plaintext summary
  used to construct Claude's compact boundary and carrier.
...
For Codex, Agent Code reads the top-level model and model provider from
`$CODEX_HOME/config.toml`, then resolves context metadata from
...
identity and the budget falls back conservatively. Model lookup failure must
happen before a target transcript is written.
...
- `src/main/providerSwitch/switchProvider.ts` — capacity policy and transaction
  order.
...
- `packages/agent-transcript-parser` — neutral conversation model, capacity
  assessment, native projectors, and real resume probe.
