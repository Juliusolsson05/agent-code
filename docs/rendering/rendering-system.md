...
Two central routes remain on purpose, and they are the complete Phase 10 list:

- `Block` still intercepts paired shell commands whose *content* proves a Git operation, and absorbs the paired result into `GitCardRow`. A structural tool fingerprint cannot distinguish `git status` from `npm test`; moving this safely requires the paired provider operation boundary, not another name switch.
- `EntryRow` still selects durable compact boundary/summary rows. Live compaction conditions and durable replayable transcript entries are different evidence planes; Phase 10 moves both to provider-owned compaction directories only after source precedence, screen-fallback provenance, deduplication, and replay fixtures exist.
...
Reviewed shape promises live in `src/providers/{claude,codex,opencode}/renderer/shapes.ts`. A catalog entry says where the structure was observed, which lifecycle milestones exist, which fixture proves it, and whether the route is specialized, generic, absorbed, condition-owned, or deliberately still planned. `scripts/audit-rendering-shapes.mts` joins the frozen bundle corpus with optional live recording sidecars. Current `fp2` unknown, misrouted, and unknown-outcome observations fail the audit; old `fp1` telemetry is reported separately because fingerprints minted by two canonicalizers are not comparable. `src/providers/shapes.coverage.test.ts` permanently gates the checked-in corpus and catalog invariants.
