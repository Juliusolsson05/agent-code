#!/usr/bin/env node
// Verify the cached cloudflared binary in
//   third_party/cloudflared/cache/<platform>-<arch>/cloudflared
// against manifest.json without re-downloading.
//
// Usage:
//   node scripts/runtime-tools/verify-cloudflared.mjs            # warn on missing
//   node scripts/runtime-tools/verify-cloudflared.mjs --strict   # fail on missing or broken
//
...
//
//   2. Runtime smoke test (native arch only): `cloudflared --version` must include
...
async function main() {
  ...
  for (const [platformKey, platform] of Object.entries(manifest.platforms)) {
    ...
    try {
      ...
      const msg = `[verify-cloudflared] missing or not executable: ${platformKey} (${binary})`
      if (strict) { console.error(msg); problems++ }
      else { console.warn(`${msg} — run \`npm run runtime:fetch:cloudflared\``) }
      continue
    }

    if (!platform.binarySha256) {
      console.error(
        `[verify-cloudflared] ${platformKey}: manifest is missing binarySha256. ` +
    ...
    if (digest !== platform.binarySha256) {
      console.error(
        `[verify-cloudflared] ${platformKey}: binary hash mismatch:\n` +
    ...
    if (platformKey !== currentPlatformKey()) {
      ...
      // it.
      console.log(`[verify-cloudflared] OK ${platformKey} (hash matches; cross-arch)`)
      continue
    }
    ...
    if (process.platform === 'darwin') {
      ...
      if (homebrewLinkage.length > 0) {
        console.error(
          `[verify-cloudflared] ${platformKey}: binary links Homebrew dylibs:\n  ` +
    ...
    console.log(`[verify-cloudflared] OK ${platformKey} (${manifest.version}, hash + linkage clean)`)
  }

  if (problems > 0) {
    console.error(`[verify-cloudflared] ${problems} problem(s) found`)
    process.exit(1)
  }
}
...
main().catch(err => {
  console.error(`[verify-cloudflared] ${err instanceof Error ? err.message : String(err)}`)
  process.exit(1)
})
