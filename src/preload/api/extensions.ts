...
export const extensionsApi = {
  extensionsSetInputBindings: (bindings: ExtensionInputBindings): Promise<void> => ipcRenderer.invoke('extensions:input-bindings', bindings),
  onExtensionNativeInput: (handler: (event: ExtensionNativeInput) => void): (() => void) => subscribe('extensions:native-input', handler),
  // Host-only view adapter. The child broker fixes the identity fields; main
  // authenticates this application frame and mints the runtime view instance.
  extensionsRuntimeCommand: (extensionId: string, revision: string, commandId: string): Promise<ExtensionJson | undefined> =>
    ipcRenderer.invoke('extensions:runtime-host', { method: 'command', extensionId, revision, commandId }),
  extensionsRuntimeAttach: (extensionId: string, revision: string, viewId: string, connectionId: string): Promise<{ instanceId: string; snapshot: RuntimeViewSnapshot }> =>
    ipcRenderer.invoke('extensions:runtime-host', { method: 'view.attach', extensionId, revision, viewId, connectionId }),
  extensionsRuntimeRequest: (connectionId: string, name: string, input: ExtensionJson): Promise<ExtensionJson | undefined> =>
    ipcRenderer.invoke('extensions:runtime-host', { method: 'view.request', connectionId, name, input }),
  extensionsRuntimeDetach: (connectionId: string): Promise<void> =>
    ipcRenderer.invoke('extensions:runtime-host', { method: 'view.detach', connectionId }),
  onExtensionsRuntimeView: (handler: (event: RuntimeViewEvent) => void): (() => void) =>
  ...
  extensionGrantedCapabilities: (id: string, revision: string): Promise<ExtensionCapability[]> =>
    ipcRenderer.invoke('extensions:granted-capabilities', id, revision),
}
