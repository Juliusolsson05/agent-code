  ...
  return [
    ...
    defineCapability({
      ...
      input: z.object({ label: z.string().regex(/^[A-Za-z]+[1-9]\d*$/).optional().describe('Exact visible label, e.g. C18. Scope with windowId; global matches may identify different agents in different windows. Resolve to a stable ID before acting.'),
        name: z.string().trim().min(1).optional().describe('Exact spoken agent name from the Agent names setting, e.g. "Apollo" or "Apollo 2". Case-insensitive and whitespace-normalized, never a substring; use query for partial text. Names are absent while the setting is off, in which case this matches nothing. A name identifies one allocation, but several windows can observe the same agent, so every candidate is still returned — resolve to sessionId before acting.'), query: z.string().default('').describe('Case-insensitive substring of agent ID, visible label, displayed/stored title, directory or provider. Empty searches all agents.'), windowId: z.string().optional().describe('Optional stable window ID from app.windows to restrict the cross-window search.'), tabId: z.string().optional().describe('Optional project tab ID from app.observe.'),
      ...
      handler: async (input, context) => {
        ...
        const rows = windows.flatMap(window => window.workspace && window.owner ? window.workspace.sessions.filter(session =>
          session.provider !== 'terminal' && (!input.label || session.displayLabel === input.label.toUpperCase())
