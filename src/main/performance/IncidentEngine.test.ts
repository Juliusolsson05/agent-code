...
describe('incident evidence and exclusion rules', () => {
  ...
  it('excuses a missing heartbeat only briefly and only for loss that can contain heartbeats', () => {
    const engine = new IncidentEngine()
    engine.loss(0, 0, 0)
    engine.reconcile([1], [1], 1000)
    engine.accept([main(1000), windowSample(1000)], 1000, 1000)
    // Operation-queue loss after the freeze cannot explain a missing heartbeat.
    engine.loss(500, 0, 3000)
    engine.accept([main(6000)], 6000, 6000)
    expect(engine.summaries()[0]).toMatchObject({ rule: 'renderer-stall', scope: 1 })

    const bounded = new IncidentEngine()
    bounded.loss(0, 0, 0)
    bounded.reconcile([1], [1], 1000)
    bounded.accept([main(1000), windowSample(1000)], 1000, 1000)
    bounded.loss(40, 40, 3000)
    bounded.accept([main(6000)], 6000, 6000)
    expect(bounded.summaries()).toHaveLength(0)
    // Silence that outlasts a loss-free heartbeat interval is reported again.
    bounded.accept([main(7100)], 7100, 7100)
    expect(bounded.summaries()[0]).toMatchObject({ rule: 'renderer-stall', scope: 1 })
  })
  it('treats the first cumulative loss count as a baseline after a helper restart', () => {
    const engine = new IncidentEngine()
    engine.loss(900, 0, 0)
    engine.accept([main(1000, 2000)], 1000, 1000)
    engine.loss(900, 0, 1000)
