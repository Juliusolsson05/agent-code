...
export type FrameDocumentInput = {
  ...
  entry: string
  /** v2 entries mount a view module; the shared runtime is attached through main. */
  viewModule?: boolean
...
export function buildFrameDocument(input: FrameDocumentInput): string {
  ...
  // (c) mount its view on the parent's signal. API v2 attaches a shared runtime
  // and imports the separate view entry; the legacy activation path stays below. Kept in one nonced module.
  const bootstrap = `
...
let runtimeState = { sequence: -1 };
const runtimeListeners = new Set();
function receiveRuntimeState(snapshot) {
  if (!snapshot || snapshot.sequence <= runtimeState.sequence) return;
  runtimeState = snapshot;
  for (const listener of [...runtimeListeners]) {
    try { listener(snapshot.state); } catch (error) { console.error('[extension] view state listener failed:', error); }
  }
}
...
window.addEventListener('message', (event) => {
  ...
  } else if (msg.kind === 'agent-code-ext:runtime-state') {
    receiveRuntimeState(msg.snapshot);
  } else if (msg.kind === 'agent-code-ext:mount') {
    if (msg.viewId !== VIEW_ID) return;
    mountRequested = true;
    tryMountRequestedView();
...
const api = {
  ...
  extension: { id: location.hostname, apiVersion: CFG.viewModule ? 2 : 1 },
  storage: {
...
// Activation and the parent's theme/mount handshake race independently. A fast
// module used to auto-mount before its first theme; a slow module ignored the
// early mount request because registerView had not run yet. Remember intent and
// require all three facts, in either arrival order. Ready means the view actually
// mounted, not merely that importing its module succeeded.
function tryMountRequestedView() {
  if (startupFailed || mounted || !activationResolved || !parentThemeReceived || !mountRequested) return;
  try {
    mountView(VIEW_ID);
...
function mountView(viewId) {
  ...
  if (!mount) throw new Error('View "' + viewId + '" was not registered during activate().');
  mounted = true;
  const root = document.getElementById('root');
  if (!root) return;
...
// Import/activation can finish before or after the parent handshake. Neither
// path may bypass the common gate or swallow a mount exception in an event
// listener; both report the same actionable failure to the host.
let extensionModule = null;
import('./' + ENTRY)
  .then(async (mod) => {
    if (CFG.viewModule) {
      const view = typeof mod.mount === 'function' ? mod : mod.default;
      if (!view || typeof view.mount !== 'function') throw new Error('View entry must export mount(element, context).');
      const attached = await request('runtime.attach', {});
      receiveRuntimeState(attached.snapshot);
      views.set(VIEW_ID, element => view.mount(element, {
        api, view: { id: VIEW_ID, instanceId: attached.instanceId },
        runtime: {
          state: () => runtimeState.state,
          request: (name, input = null) => request('runtime.request', { name, input }),
    ...
    // The SDK's defineExtension helper documents a default-exported module;
    // plain authored bundles often use named activate/deactivate exports. Accept
    // both public forms, retaining the same object for matching teardown.
    extensionModule = typeof mod.activate === 'function' ? mod : mod.default;
    if (!extensionModule || typeof extensionModule.activate !== 'function') {
      throw new Error('Extension entry must export activate(context) or a default extension module.');
    }
