...
/** Newest API major; v1 retains its explicit per-view compatibility adapter. */
...
export const extensionManifestSchema = z.object({
  ...
  for (const [index, view] of (manifest.contributes?.views ?? []).entries()) {
    if (!view.entry) context.addIssue({ code: 'custom', path: ['contributes', 'views', index, 'entry'], message: 'API v2 views require a separate mount module entry' })
  }
})
...
export function parseExtensionManifest(raw: string): ExtensionManifest {
  ...
  // "needs API v3, this build has v2" instead of a generic field error. This is the
...
export function apiVersionMismatch(apiVersion: number): string | null {
  if (apiVersion === 1 || apiVersion === SUPPORTED_API_VERSION) return null
