...
const MAX_BUNDLE_FILES = BUNDLE_MAX_ENTRIES
...
/**
 * Stage on the same filesystem as immutable generations so publication can use
 * rename. The dot prefix cannot be an extension ID and is never served as code.
 * The live set also prevents a cleanup call during consent from deleting work
 * still owned by an installer; startup normally runs before any windows exist.
 */
...
async function makeStagingDir(): Promise<string> {
  return withLedgerLock(async () => {
    ...
    const path = await mkdtemp(join(EXTENSIONS_DIR, '.staging-'))
    activeStagingDirectories.add(path)
    return path
  })
}
...
async function finalizeInstall(
  ...
  // Hash every served file before publication. Provenance is useful for source
  // diagnostics, but the grant must be bound to bytes we can re-check on disk.
  const bundleSha256 = await computeBundleHash(bundleDir)
  const record: InstalledExtension = {
    manifest,
    ...provenance,
    installation: { id: randomUUID(), bundleSha256 },
  ...
  await withLedgerLock(async () => {
    ...
    // Neither the previous bytes nor their approval is changed before it. A
    // process crash before the rename leaves an unreferenced new directory;
    // after it, a complete bundle and its consent are already published together.
    // There is no backup to guess about and no second grant-file write to lose.
    await rename(bundleDir, finalDir)
    try {
      await writeLedger(rows.filter(row => row.manifest.id !== manifest.id).concat(record))
    } catch (error) {
      await discardExtensionBundle(record)
      throw error
    }
 ...
 * Old `.replacing-*` backups are intentionally preserved: that installer did not
 * record a commit phase, so deleting one could delete its last recoverable copy.
 */
export async function sweepAbandonedInstallDirectories(): Promise<void> {
  await withLedgerLock(async () => {
    let entries: string[]
    try { entries = await readdir(EXTENSIONS_DIR) } catch (error) {
...
export async function installExtension(
  ...
  try {
    ...
    activeStagingDirectories.delete(work)
  }
}
  ...
  try {
    ...
    await cp(sourceReal, staging, {
      ...
      filter: async src => {
        ...
        if (rel === '') return true
        // Bound before cp copies the next file. The post-copy hash remains the
        // authoritative integrity check, but cannot undo disk exhaustion from
        // an enormous local source or a source that grows during development.
        if (++copiedEntries > BUNDLE_MAX_ENTRIES || parts.length > BUNDLE_MAX_DEPTH + 1) {
          throw new InstallError('Extension source has too many files or nested directories.')
        }
        const stats = await lstat(src)
        if (stats.isFile() || stats.isSymbolicLink()) copiedBytes += stats.size
        else if (!stats.isDirectory()) throw new InstallError('Extension source contains a special file.')
        if (copiedBytes > BUNDLE_MAX_BYTES) throw new InstallError('Extension source exceeds 64 MiB. Choose a built bundle.')
        return true
      },
    })
    await assertBundleTreeIsSafe(staging)
