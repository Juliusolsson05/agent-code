...
export function handleExtensionScheme(target: Protocol = protocol): void {
  // A managed background runtime has its own browser session. Scheme handlers
  // belong to sessions, not to the application as a whole; omitting this adapter
  // would make modules work in views but fail in the isolated runtime partition.
  target.handle(EXTENSION_SCHEME, async request => {
    let url: URL
    try {
    ...
    // Every request needs an installed owner, including assets. A directory
    // left by a failed install or an uninstall is not authority to serve code.
    const record = await withLedgerLock(async () =>
      (await readLedger()).find(row => row.manifest.id === extensionId))
    if (!record) return new Response('not found', { status: 404 })
    const assetPrefix = `__bundle/${extensionRevision(record)}/`
    // Put the document itself under the generation prefix too. Relative assets
    // in CSS or fetch('./asset.json') then resolve in the same bundle as import
    ...
    if (relative === RUNTIME_DOCUMENT) {
      ...
      return new Response(buildRuntimeDocument(record.manifest, nonce), {
        headers: {
    ...
    if (relative === RESERVED_FRAME_PATH) {
      ...
      if (url.searchParams.get('rev') !== extensionRevision(record)) {
        return new Response('extension was updated; reload the view', { status: 409 })
      }
      ...
      const declaresView = (record.manifest.contributes?.views ?? []).find(v => v.id === viewId)
      ...
      const html = buildFrameDocument({
        extensionId,
        viewId,
        entry: record.manifest.apiVersion === 2 ? declaresView.entry! : record.manifest.entry,
        viewModule: record.manifest.apiVersion === 2,
    ...
    // Imports and lazy chunks carry their immutable generation in the URL.
    // Without it an old frame could import a new chunk after update, mixing two
    // releases in one runtime while still holding the old approval snapshot.
    const root = extensionBundleDirectory(record)
