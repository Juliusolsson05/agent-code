...
describe('per-extension secret store', () => {
  it('round-trips through encrypted per-key files that only the owning extension id addresses', async () => {
    ...
    await secrets.set('example', 'example.apiKey', KEY)
    await expect(secrets.get('example', 'example.apiKey')).resolves.toBe(KEY)
    await expect(secrets.get('timer', 'example.apiKey')).resolves.toBeNull()
    const files = await readdir(join(root, 'example'))
    expect(files).toHaveLength(1)
    const raw = await readFile(join(root, 'example', files[0]))
    expect(raw.toString('utf8')).not.toContain(KEY)
    expect((await stat(join(root, 'example', files[0]))).mode & 0o777).toBe(0o600)
    await secrets.delete('example', 'example.apiKey')
    await expect(secrets.get('example', 'example.apiKey')).resolves.toBeNull()
  })
  ...
  it('refuses to store anything when the OS cannot encrypt — never a plaintext fallback', async () => {
    ...
    const error = await secrets.set('example', 'example.apiKey', KEY).then(() => null, (reason: Error) => reason)
  ...
  it('enforces key grammar, value bounds and the per-extension key limit without echoing values', async () => {
    ...
    for (const key of ['', 'has space', 'x'.repeat(65), 'slash/key']) {
      await expect(secrets.set('example', key, 'v')).rejects.toThrow(/Secret keys/)
    }
    const oversized = `${KEY}${'x'.repeat(4096)}`
    const error = await secrets.set('example', 'big', oversized).then(() => null, (reason: Error) => reason)
    expect(error?.message).toMatch(/1-4096/)
    expect(error?.message).not.toContain(KEY)
    await Promise.all(Array.from({ length: MAX_SECRET_KEYS_PER_EXTENSION }, (_, i) => secrets.set('example', `k${i}`, 'v')))
    await expect(secrets.set('example', 'one-more', 'v')).rejects.toThrow(/at most/)
    // Replacing an existing key is not a new key.
    await expect(secrets.set('example', 'k0', 'v2')).resolves.toBeUndefined()
  })

  it('keeps "." and ".." keys and case variants inside distinct files (hex names)', async () => {
    const { root, secrets } = await store()
    await secrets.set('example', '..', 'dots')
    await secrets.set('example', 'Token', 'upper')
    await secrets.set('example', 'token', 'lower')
    await expect(secrets.get('example', '..')).resolves.toBe('dots')
    await expect(secrets.get('example', 'Token')).resolves.toBe('upper')
    await expect(secrets.get('example', 'token')).resolves.toBe('lower')
    await expect(readdir(root)).resolves.toEqual(['example'])
  })
  ...
  it('deletes an extension\'s secrets on uninstall and refuses a traversal id', async () => {
    ...
    await removeExtensionSecrets('example', root)
    await expect(secrets.get('example', 'token')).resolves.toBeNull()
