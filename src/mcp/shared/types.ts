...
const BUILT_IN_MCP_DOMAINS_BY_PROVIDER = {
  ...
  // Pi has no MCP client of its own. The same HTTP endpoints reach it through
  // the bridge extension's tool proxy (addPiBuiltInMcpLaunchConfig), which
  // registers every tool as `mcp__<server>__<tool>`, so the full set is
  // offered as for the other terminal providers.
  pi: [...BUILT_IN_MCP_DOMAINS],
