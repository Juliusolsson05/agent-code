...
describe('generated per-provider split commands', () => {
  ...
  it('accounts for the difference between literal and total command count', () => {
    // 119 total - 6 generated = 113 literal `id:` fields across the command
...
describe('governance targets', () => {
  ...
  it('lands on the arithmetic the plan predicted', () => {
    // 102 baseline - 30 retirements + 47 additions = 119, checked against the
    ...
    // counted as stay in this total.
    expect(builtInCommandCatalog.length + RETIRED_COMMAND_IDS.length - 47).toBe(102)
    expect(builtInCommandCatalog).toHaveLength(119)
  })
})
