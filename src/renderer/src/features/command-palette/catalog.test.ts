...
// Merge Project Tabs (#913), 123 with View TLDR History (#917), and 125 with
// Goal preview and Goal MCP (#936).
...
describe('built-in command catalog — baseline characterization', () => {
  ...
  it('has exactly 125 commands', () => {
    ...
    // TLDR History (#917) → 125 with Goal preview and Goal MCP (#936).
...
describe('generated per-provider split commands', () => {
  ...
  it('accounts for the difference between literal and total command count', () => {
    // 125 total - 4 generated = 121 literal `id:` fields across the command
    ...
    // the count test above (through Goal preview and Goal MCP, #936).
    expect(builtInCommandCatalog.length - nonDefaultProviders.length * 2).toBe(121)
  })
...
describe('governance targets', () => {
  ...
  it('lands on the arithmetic the plan predicted', () => {
    ...
    expect(builtInCommandCatalog.length + RETIRED_COMMAND_IDS.length - 28).toBe(102)
    expect(builtInCommandCatalog).toHaveLength(125)
  })
})
