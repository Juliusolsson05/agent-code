...
export function TldrFreshness({ runtime, writtenAt, writtenLabel = 'Note written', completedAt, enforcementInactive = false }: { runtime?: SessionRuntime; writtenAt?: string; writtenLabel?: string; completedAt?: string; enforcementInactive?: boolean }) {
  ...
  return <div data-tldr-freshness="" className="absolute inset-x-4 bottom-4 flex flex-wrap justify-center gap-x-4 gap-y-1 text-[11px] leading-4 text-muted">
    ...
    {/* A third stamp rather than replacing "Goal set" (#1182): how long an
        agent has sat finished is the signal for closing it, and how long ago
        the goal was set still says how long the work took. */}
    {completedAt && <Timestamp label="Completed" timestamp={Date.parse(completedAt)} now={now} />}
