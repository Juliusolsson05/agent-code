...
// never keep its own copy. Since the UI pass (G-17) the two cards are a real
// radio group (they were toggle buttons), so "which is on" is aria-checked.
...
describe('Update channel row', () => {
  it('shows the channel main reports and the running version', async () => {
    ...
    expect(screen.getByRole('radio', { name: /^Stable/ }).getAttribute('aria-checked')).toBe('false')
