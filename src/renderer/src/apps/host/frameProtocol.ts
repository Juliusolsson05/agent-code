...
export const frameRequestSchema = z.discriminatedUnion('method', [
  z.object({ method: z.literal('runtime.attach') }).strict(),
  z.object({ method: z.literal('runtime.request'), name: z.string().regex(/^[a-zA-Z][a-zA-Z0-9_.-]{0,63}$/), input: extensionJsonSchema }).strict(),
  z.object({ method: z.literal('storage.get'), key: SAFE_STORAGE_KEY }),
  z.object({
    method: z.literal('storage.set'),
    key: SAFE_STORAGE_KEY,
    value: extensionJsonSchema,
  }),
...
export type FramePush =
  | { kind: 'agent-code-ext:runtime-state'; snapshot: RuntimeViewSnapshot }
