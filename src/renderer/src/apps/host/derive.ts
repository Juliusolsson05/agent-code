  ...
  for (const entry of installed) {
    ...
    for (const command of entry.manifest.contributes?.commands ?? []) {
      ...
      commands.push({
        ...
        description: command.description?.trim() || entry.manifest.description.trim() || entry.manifest.name,
        ...
        run: ({ ui }) => {
          ...
          if (entry.manifest.apiVersion === 2) {
            // Main owns the engine independently of visible panes. Return its
            // acknowledgement to the command gateway so errors are visible and
            // a cold command neither opens a view nor runs a second activation.
            ui.closePalette()
            return window.api.extensionsRuntimeCommand(entry.manifest.id, extensionRevision(entry), command.id).then(() => {})
          }
          // Legacy action command. It runs inside the extension's live frame — the ONLY
