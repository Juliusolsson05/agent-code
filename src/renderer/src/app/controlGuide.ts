...
Use views.preferencesRead for an exact agent’s configured/effective display and follow state; views.modeSet and views.followSet require its revision. views.tailAllSet controls Tail All; enabling it clears Tail Working. Inspect tailAll/tailWorking because either bulk mode can keep followEnabled true after disabling an individual preference. Tail Working excludes agents waiting for human input. To change it, use commands.run with commandId toggle-tail-working, then read preferences again; it is a toggle, so never retry an uncertain invocation blindly. Preference revisions include effective state and can change as activity changes; re-read after stale_cursor. These tools preserve lanes. Agent View Mode selects a rendered conversation, native terminal, or Hybrid behavior. Hybrid uses terminal presentation as its resting surface while features may temporarily require rendered interaction. A per-session override and a provider's terminal runtime are separate choices; inspect the actual supported capability before invoking a rendered-only action.
...
Closing an agent can affect related children, while removing a lane or hiding a pane need not close the agent. Closing a project's root agent while other sessions exist asks whether to close only that agent (Close Agent, or Close Terminal for a terminal) or the whole tab (Close Tab); choose Close Agent unless the user asked to close the whole project. Use the app's close preview/confirmation and undo behavior; do not infer that every close is reversible. Interrupt, stop, close, bury and detach are distinct operations. Persistent terminals may use tmux and have different recovery semantics from provider agents.

Agent names are an optional spoken address, off by default and toggled by the "Agent names" setting. When enabled, each agent and terminal gets one stable name from a fixed ranked list (Apollo, Jasper, Beatrix, …); past 100 agents the names take explicit numeric suffixes such as "Apollo 2". A name is application-wide and survives restart, reload, provider replacement and restore, so it is unlike displayLabel, which is window-local and changes with layout. It is never reused after an agent closes, so a request for a closed name resolves to nothing rather than to a different agent. A name can belong to a terminal (provider "terminal"): drive it with terminals.read/terminals.input, never agents.prompt, which refuses terminals. Search a name with agents.search {name: "Apollo"}: the match is exact and case-insensitive, not a substring, and it still returns every candidate window for that agent, so resolve to the stable session ID before acting. While the setting is off, agentName is null everywhere and a name search matches nothing; the operator server itself is unaffected.`,
  },
  {
...
Use terminals.create/read/input for project terminals and bounded raw PTY replay. Terminals are sessions too: agents.locate/show/close/restore/list/titleSet/pinSet and agents.search accept them (provider "terminal"), but agents.prompt refuses them. Send text to a terminal, including one found by its spoken name, with terminals.input. Reads never attach a view or wake a process; input requires the observed backend run ID and appends no Enter. Use agents.close for the normal confirmation/cascade flow, then operations.read with its callId for completion. For a project's root agent or terminal with other sessions in the project, that flow asks Close Agent (Close Terminal) vs Close Tab; choose Close Agent unless the user asked to close the whole project.

The external operator connection is in Settings → MCP → External Operator MCP. It is off by default, listens only on this computer and applies to all Agent Code windows. Enabling it installs an app-managed global Codex MCP connection and the agent-code-computer-execution skill in the selected Codex home. Agent Code updates the connection on port/key changes. Disabling stops the server and removes its skill and connection credential; per-tool approval preferences survive in a disabled Codex entry. Those preferences are preserved across setup, key rotation and re-enable. Edits to app-owned connection fields or the managed skill, and unmanaged files, are preserved and reported as conflicts. Restart the external client after setup or key rotation. Internal Agent Code sessions and workflow workers exclude the operator connection and skill. Its reserved server name is agent-code-control; JSON copy is available for other manually configured local clients.
