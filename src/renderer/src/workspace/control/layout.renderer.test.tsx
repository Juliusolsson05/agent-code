...
it('preserves recorded workspace identities through row edits and refuses a stale positional action', async () => {
  ...
  const mounted = renderHook(() => {
    ...
    const dispatch = useDispatchActions(state, store.setWorkspaceState, () => {}, refs, vi.fn(), () => {})
...
it('reports effective tiled focus separately from remembered classic selection after lane replacement and removal (#798)', async () => {
  useAppStore.setState({ workspaceState: structuredClone(fixture.state) as unknown as WorkspaceState, workspaceReaderMode: null, workspaceSpotlight: null })
  const mounted = renderHook(() => {
    const state = useAppStore(store => store.workspaceState)
    const refs = useRef(makeRefs(state)).current
    refs.stateRef.current = state; refs.latestStateRef.current = state
    const store = useAppStore.getState()
    return { ...useDispatchActions(state, store.setWorkspaceState, () => {}, refs, vi.fn(), () => {}), restoreStatus: 'fresh' }
  })
