...
export function draftControlCapabilities(getWorkspace: () => Workspace) {
  return [
    defineCapability({
      ...
      handler: input => {
        const { runtime, summary } = inspectAgentDraft(input.sessionId)
    ...
    defineCapability({
      ...
      handler: input => {
        ...
        const before = inspectAgentDraft(input.sessionId)
        ...
        const after = inspectAgentDraft(input.sessionId).summary
