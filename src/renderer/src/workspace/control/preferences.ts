...
const output = z.object({ sessionId: z.string(), override: z.enum(['agent', 'terminal']).nullable(), globalMode: z.enum(['agent', 'terminal', 'hybrid']), effectiveSurface: z.enum(['rendered', 'terminal']), autoFollow: z.boolean(), tailAll: z.boolean(), followEnabled: z.boolean(), revision: z.string() })
export function preferenceControlCapabilities(getWorkspace: () => Workspace) {
  const read = (sessionId: string) => {
    ...
    const value = { sessionId, override: meta.agentViewModeOverride ?? null, globalMode: state.settings.agentViewMode,
      ...
      autoFollow: runtime.tailMode, tailAll: state.tailAllMode, followEnabled: runtime.tailMode || state.tailAllMode }
