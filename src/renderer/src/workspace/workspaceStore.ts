...
// -----------------------------------------------------------------------------
// Extracted pure modules (2026-04-23 refactor — see task #5)
//
// Everything below the hook used to live inline in this file. Pure
// functions that the hook calls but doesn't close over got lifted
// into focused sibling modules:
//
//   ./semantic/          foldSemanticEvent + helpers + summarizers
//   ./codex/             rollout mapper, factories, envelope stripping
//   ./claude/            history-marker / progress-entry unwrapping
//   ./entries/utils      entry text extraction + tool-use indexing
//   ./runtime/feedDebug  the appendFeedDebugLog helper
//   ./spawn/             session-spawn error message normalizer
//   ./layout/helpers     titleFromCwd / ratio math / pickerEqual /
//                        setRatioBetween / sanitizeTileTabsState
//   ./persistence        PersistedWorkspace type
//
// The hook itself stayed here — it owns per-session refs, IPC
// subscriptions, and cross-cutting mutations (spawn+tree+sessions
// happen together), and splitting it further would either require
// sharing refs across modules or routing everything through a new
// state library. The WHY comments for every extracted chunk moved
...
//
// Previous callers imported these symbols from workspaceStore.ts; the
// 2026-04-23 extraction moved the implementations into sibling modules
// but the re-exports keep the import paths in ProxyDebugPanel,
// claudeSession, Feed.tsx, ghosts.ts, and the testing harness
// working verbatim. Prefer importing from the new modules directly
// in new code.
// -----------------------------------------------------------------------------
