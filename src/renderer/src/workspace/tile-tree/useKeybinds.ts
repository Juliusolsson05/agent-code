...
export function useKeybinds(
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      //
      // WHY this specific chord: cmd+e is taken by the existing
      // tile-resize chord, cmd+shift+e is unused, and it mirrors
      // VS Code's "Explorer" muscle memory for users coming from
      // an IDE. The toggle is global — no `when` guard, no mode
      // dependence — because Global Editor is orthogonal to
      // dispatch / tile / spotlight (it WRAPS them rather than
      // replacing them).
