...
//
// Agent Completion Indicator (#1172, redesigned in #1200): a pane whose
// finished turn is still unseen draws the working bar HOLLOW. It keeps the
// surface background, gets a full-accent ring, and its text turns accent.
// Filled means busy and outlined means done-go-look: the same bar in the same
// colour, so the two read as one family across the grid and can't be
// confused. (Stripes were tried twice and dropped; see
// .pane-header-completion-outline for why.) "Unseen" is the session's unread
// marker, the same one behind the Dispatch NEW badge. The leaf clears it on
// engagement or after a dwell (useAcknowledgeAfterDwell), never on a passing
// focus.
//
...
export function PaneHeader({
   ...
   *  so a shell TerminalLeaf never shows the outline. */
  ...
  // WHY a running agent is never outlined: the outline says "finished, go look", and a
  ...
  // fill and the outline can never share a row. Deliberately the same
  ...
  // outline a pane that has just started streaming. Matching the fill is what
  // keeps the two from ever overlapping, and it's the smaller cost.
  const completionOutlined = completionIndicator && completionUnseen && !isSessionLive
  return (
    <div className="border-b border-border bg-surface text-muted font-code select-none">
      <div
        ...
        data-completion-outlined={completionOutlined ? 'true' : 'false'}
        // The outline class overrides `bg-surface text-muted` because it is
        // UNLAYERED CSS in styles.css, and unlayered rules beat Tailwind's
        // `@layer utilities` whatever the order of the class attribute. It
        // would beat `bg-accent` the same way, which is harmless only because
        // the lit fill and the outline are mutually exclusive (see
        // completionOutlined above). Keep them so.
        className={`flex items-center justify-between text-[10px] ${
          statusLit ? 'bg-accent text-accent-fg' : 'bg-surface text-muted'
        } ${completionOutlined ? 'pane-header-completion-outline' : ''} ${statusMode ? 'min-h-[5px]' : ''}`}
      >
