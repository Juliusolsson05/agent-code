  ...
  const spawn = useCallback(
    async (
      ...
      try {
        ...
        // No ghost-log read here any more (#1225). This `sessionId` is the id
        // main minted for this spawn a moment ago, so its log can only be
        // empty; the read that looked like crash-resume never found anything
        // after the July recovery rewrite moved restores onto `session:recover`
        // with the persisted id. The one reader is now the initial-history
        // load above, which runs for restored panes too and reconciles the
        // log against the transcript it just loaded.
