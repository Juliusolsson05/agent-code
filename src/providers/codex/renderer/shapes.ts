...
export const CODEX_RENDER_SHAPES = defineRenderShapeCatalog('codex', {
  ...
  'codex.semantic.exec.v1': defineRenderShape({
    ...
    alternateDispositions: [
      ...
      { kind: 'specialized', rendererId: 'shared.command', protocolId: 'command.search' },
      { kind: 'specialized', rendererId: 'shared.command', protocolId: 'command.read' },
      { kind: 'specialized', rendererId: 'shared.command', protocolId: 'command.list' },
    ],
    why: 'Unified exec is admitted only after bounded provider parsing proves a patch, command, Git workflow, numbered command group, or one top-level awaited Agent Code MCP call with JSON-compatible literal input. Embedded MCP identity is specialized without absorbing arbitrary projected output; scripts with dynamic arguments, multiple calls, executable templates, or unproven prefixes remain generic. Direct text(output) command carriers retain an internal unknown status and display "exit code unavailable" rather than inventing success.',
  }),
  ...
  'codex.semantic.wait.v1': defineRenderShape({
    ...
    why: 'The captured wait grammar carries a non-empty cell_id plus non-negative JavaScript-safe-integer controls. It renders as a continuation, names terminate requests explicitly, and treats yielded envelopes as running rather than successful completion. Transport chrome is stripped only from its correlated Codex result; malformed prefixes remain generic and no result is attached to a different command.',
  }),
  ...
  'codex.tool-result.custom-tool-call-output.v1': defineRenderShape({
    ...
    fixtures: { final: ["rendering-bundles/2026-05-20T15-15-32-562-7b859c43.json","rendering-bundles/2026-05-20T18-43-47-377-f6d733f8.json","rendering-bundles/2026-05-21T09-12-59-519-e4f30416.json","rendering-shapes/codex/exec/committed.json","rendering-shapes/codex/wait/committed.json"], prefixes: [] },
    ...
    alternateDispositions: [
      ...
      { kind: 'absorbed', ownerRendererId: 'codex.rows.dispatch', protocolId: 'agent-code.embedded-operation', reason: 'An empty successful embedded-operation acknowledgement is represented by its named invocation row.' },
      ...
      { kind: 'absorbed', ownerRendererId: 'shared.command', protocolId: 'command.search', reason: 'The paired Search operation preserves the bounded result evidence.' },
      { kind: 'absorbed', ownerRendererId: 'shared.command', protocolId: 'command.read', reason: 'The paired Read operation preserves the bounded result evidence.' },
      { kind: 'absorbed', ownerRendererId: 'shared.command', protocolId: 'command.list', reason: 'The paired List operation preserves the bounded result evidence.' },
    ],
    why: 'GRADUATED Phase 10: correlation at the paired operation boundary determines the legal route. July 18 invocation/result fixtures pin the embedded-operation and wait receipts, including their empty-success absorption. The open-world output envelope itself never grants a specialized or hidden claim.',
  }),
  ...
  'codex.tool-use.wait.v1': defineRenderShape({
    ...
    why: 'The durable wait fixture proves a correlated Codex continuation envelope. Controls are admitted only as non-negative JavaScript-safe integers, termination is named explicitly, and a yielded result remains running. Only its paired result may lose the verified Script/Wall time/Output transport header; drifted inputs remain generic.',
  }),
