...
export class ClaudeSession extends EventEmitter {
  ...
  private readonly promptAcceptanceWaiters = new Set<{
    /**
     * Per-filter rejection tallies for THIS waiter, reported on timeout.
     * Exists because the 2026-08-30 false timeouts were undiagnosable from
  ...
  armPromptAcceptance(
    ...
    const waiter = {
      ...
      armedAt: Date.now(),
      misses: { cursor: 0, timestamp: 0, image: 0, exact: 0 },
    ...
    const timer = setTimeout(
      () => waiter.finish({ kind: 'timeout', nearMisses: waiter.misses }),
  ...
  private resolvePromptAcceptance(entry: JsonlEntry, cursor: number): void {
    ...
    for (const waiter of [...this.promptAcceptanceWaiters]) {
      // Every rejection is tallied on the waiter so a timeout can report WHICH
      // filter starved it (see the `misses` field comment). The tallies are
      // diagnostic only — they never change what matches.
      if (cursor <= waiter.afterCursor) { waiter.misses.cursor += 1; continue }
      ...
      if (Number.isFinite(entryTimestamp) && entryTimestamp < waiter.armedAt) {
        waiter.misses.timestamp += 1
        continue
      }
      if (waiter.requiresImage && kind === 'user' && !hasImageContent) {
        waiter.misses.image += 1
        continue
      }
      if (
      ...
      ) { waiter.misses.image += 1; continue }
...
/**
 * Claude's editor does not preserve the bytes we send, so byte-exact matching
 * reports successful deliveries as failures. The transforms are not guesses —
 * each is pinned to a line of Claude Code's own source (vendored under
 * vendor/claude-code-src) and/or to recorded evidence:
 *
 * - TAB -> four spaces, literally (`.replaceAll('\t', '    ')`,
 *   components/PromptInput/PromptInput.tsx:1204). Proven live by the
 *   2026-08-30 22:07:38 false timeout (debug bundle
 *   2026-08-30T22-07-43-944-88c29f92): a pasted table's 8 tabs came back as
 *   space runs in BOTH the queue-operation and user entries, and in the API
 *   request body — the transform happens the moment a paste enters Claude's
 *   composer, so no transcript witness can ever equal the sent bytes.
 *   (Adversarial verification disproved the tab-stop theory: a draft's
 *   space+tab became exactly 5 spaces, which only the literal 1->4 rewrite
 *   reproduces.)
 * - Every CR -> LF (`.replace(/\r/g, '\n')`, same line), which DOUBLES a
 *   CRLF into two newlines — the old `\r\n? -> \n` step here diverged from
 *   that and could itself manufacture a mismatch.
 * - Full trim on the queue path only (`finalInput.trim()`,
 *   utils/handlePromptSubmit.ts:337) while direct submission keeps ends
 *   intact — so the canonical form must trim BOTH ends or a mid-turn
 *   delivery of a prompt with leading whitespace re-creates this bug.
 * - NFC normalization for short prompts: non-paste-like deliveries (<=100
 *   chars, no newline) are typed through Claude's Cursor, whose MeasuredText
 *   does `text.normalize('NFC')` (utils/Cursor.ts:1135). NFD input — macOS
 *   filenames, PDF copy-paste — commits as NFC and byte-mismatches.
 *
 * Hence: NFC-normalize, collapse every whitespace run to one space, trim.
 * The corpus measurement that forced this: ALL 21 recorded
 * acceptance-timeouts (2026-08-03..08-30) had an in-window transcript entry —
 * a 100% false-failure rate, each one restoring a stale draft over the
 * user's composer and arming the resend guard against a prompt Claude
 * already had.
 *
 ...
 * still mismatch; nothing in the app composes such prompts, and widening for
 * it would mean matching on a lossy projection of both sides.
 */
