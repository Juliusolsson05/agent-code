...
describe('provider kind source of truth', () => {
  it('lists exactly the wired agent providers', () => {
    expect([...AGENT_PROVIDER_KINDS]).toEqual(['claude', 'codex', 'opencode', 'grok', 'pi'])
  })
...
describe('process operations after terminal parity', () => {
  it('keeps shells and agents eligible while excluding extension views', () => {
    expect(isProcessSessionKind('terminal')).toBe(true)
    expect(isProcessSessionKind('extension-view')).toBe(false)
    expect(isProcessSessionKind(undefined)).toBe(true)
    for (const provider of AGENT_PROVIDER_KINDS) {
      expect(isProcessSessionKind(provider)).toBe(true)
    }
  })
})
