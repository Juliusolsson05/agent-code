...
/**
 * Does this pane own a provider or shell process?
 *
 * Terminal parity made pins, output following, and prompt-template delivery
 * available to shells as well as agents. Reusing the extension branch's old
 * agent-only guards while integrating main would silently undo that behavior.
 * Conversely, accepting every SessionKind would send those operations to an
 * extension view with no PTY. Keep this partition explicit and positive so a
 * future processless pane cannot acquire process operations by default.
 * An absent kind still means the historical Claude session, as above.
 */
export function isProcessSessionKind(value: SessionKind | undefined): boolean {
  return value === 'terminal' || isAgentSessionKind(value)
}
