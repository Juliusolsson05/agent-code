...
import { parseMonitorIncident } from './parseMonitorIncident.js'
...
export function parseMonitorHistoryPoint(value: unknown): MonitorHistoryPoint | null {
  ...
  if (main !== null && (!object(main) || Object.keys(main).length !== 6 || !nullable(main.cpuPercent)
    || !finite(main.rss) || !finite(main.heapUsed) || !finite(main.heapLimit)
    || !nullable(main.loopP99Ms) || !nullable(main.loopMaxMs))) return null
...
function parsePage(value: unknown): MonitorHistoryPage | null {
  if (!object(value) || Object.keys(value).length !== 7 || !['1s', '10s', '1m'].includes(value.resolution as string)
  ...
  if (!status || points.some(point => point === null)) return null
  return { ...value, points: points as MonitorHistoryPoint[], status } as MonitorHistoryPage
}
...
export function parseMonitorWorkerQueryResult(value: unknown): MonitorWorkerQueryResult | null {
  ...
  if (value.kind === 'incident') {
    if (value.value === null) return { kind: 'incident', value: null }
    const incident = parseMonitorIncident(value.value)
    return incident ? { kind: 'incident', value: incident } : null
  }
